                ...
                while (run) {
                    if (empNoList.get(idx) != null) {
                        ...
                        for (AttendDto attendDto : attendDtoList) {
                            String date = attendDto.getAtdDate();
...
//              ?어라 오늘날짜에 휴가가 있네? 그럼 뭐 해야해?? 오늘날짜의 근태코드랑 근태수를 가져와야지
                            ...
                            if (updateResult>0){
                                System.out.println("workScheduleDao.updateWorkSchedule(workScheduleDto) gooood!");
                            }
                        }
                        idx++;
                    } else if (empNoList.get(idx) == null) {
                        run = false;
                    }
                }
            }
//            attendDao.getAttendListByEmpNo()
        ...
        // 이렇게 하면 오늘자 사원들의 워크 스케쥴러가 만들어지면서, 휴가도 업데이트된다.
        // 이제 로그인한 사람의 출퇴근 기록을 한다.

        //empNo(로그인한 사람의 사원 번호)를 통해서 그 사람의 workScheduleDto를 가져와!
        WorkScheduleDto workScheduleDto  =workScheduleDao.getWorkScheduleOne(empNo);
